...
fn main() {
    ...
    let mut output = matches.value_of("output_maf").map(|p| Box::new(File::create(p).expect("Couldn't create output file")) as Box<Write>).unwrap_or(Box::new(stdout()));
